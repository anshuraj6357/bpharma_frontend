...
// const handleLogout = () => {
//   localStorage.clear();       // user + token clear
//   appStore.dispatch({ type: "auth/userLoggedout" });
// };
...
export default function Header() {
  ...
  const handleLogout = async () => {
    try {
      await logoutUser().unwrap(); // backend logout (optional)
    } catch (err) {
      console.log("Backend logout failed, continuing frontend logout");
    }

    // ✅ frontend logout (MOST IMPORTANT)
    localStorage.removeItem("user");
          ...
          <div className="hidden md:flex items-center gap-4">
            {!isAuthenticated ? (
              ...
              <div ref={dropdownRef} className="relative">
                ...
                {openDropdown && (
                  <div className="absolute right-0 mt-3 w-52 bg-white shadow-lg rounded-xl">
                    <button onClick={() => navigate("/myprofile")} className="menu-btn">My Profile</button>
                    <button onClick={() => navigate("/mybooking")} className="menu-btn">My Bookings</button>
