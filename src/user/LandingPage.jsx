import { useState,lazy, useEffect,Suspense, useRef, useCallback, useMemo } from "react";
...
/* ---------------- SAFE DATA MERGE ---------------- */
useEffect(() => {
  if (!pgApiData?.data?.length) return;

  setPgData(prev => {
    const map = new Map(prev.map(p => [p._id, p]));
    pgApiData.data.forEach(p => map.set(p._id, p));
    return Array.from(map.values());
  });

  if (pgApiData.nextCursor) {
    setCursor(pgApiData.nextCursor);
    setHasMore(true);
  } else {
    setHasMore(false);
  }
  ...
  const observer = new IntersectionObserver(
    ([entry]) => {
      if (entry.isIntersecting && hasMore && !isFetching) {
        // cursor change triggers fetch automatically
      }
    },
    { rootMargin: "200px" }
  );
...
const processedPgData = useMemo(() => {
  ...
  const filtered = pgData.filter(pg => {
    switch (activeFilter) {
      case "Popular":
        return (pg.rating || 0) >= 4.2;
      case "Newest":
        return (
          new Date(pg.createdAt) >
          Date.now() - 30 * 24 * 60 * 60 * 1000
        );
      case "Budget":
        return pg.price < 15000;
      default:
        return true;
    }
  });
...
useEffect(() => {
  const title =
    "RoomGi - Premium PG & Rooms Without Brokerage | Verified Stays";
  const description =
    "Discover verified PGs, rooms & shared accommodations with zero brokerage.";
  ...
  <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-indigo-50 font-inter selection:bg-indigo-100/60">
    ...
    >
      <div className="absolute inset-0">
          <img
            src={heroimg}
            fetchPriority="high"
            decoding="sync" // Fastest possible render
            className="w-full h-full object-cover brightness-50"
            alt="Modern luxury living spaces for rent"
            // Ensure this image is not lazy loaded
          />
    ...
    {hasMore && (
      <div ref={loadMoreRef} className="h-24 flex items-center justify-center">
        {isFetching && <LandingPageSkeleton />}
      </div>
    )}
