...
const PropertyCard = React.memo(({ property, onDelete, deletingPropertyId, totalOccupancyRate, totalVacant, occupiedBeds, occupiedRental, totalpgbed, totalRooms, totalOccupied }) => (
  <motion.div
    layout
    ...
    <div className="bg-white rounded-[2rem] border border-slate-100 shadow-[0_10px_30px_-15px_rgba(0,0,0,0.05)] hover:shadow-[0_30px_60px_-20px_rgba(0,0,0,0.12)] transition-all duration-500 overflow-hidden h-full flex flex-col">
      <div className="relative h-48 overflow-hidden">
        <img
          ...
          alt={`${property.name} - ${property.city} property`} // SEO alt text [web:2]
      ...
      <div className="p-6 flex flex-col flex-1 bg-[#fdfdfd]">
        ...
        <div className="mt-auto pt-6 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button className="p-2.5 rounded-xl bg-white border border-slate-200 text-slate-400 hover:text-indigo-600 hover:border-indigo-200 hover:bg-indigo-50 transition-all active:scale-90" title="Edit Property">
              <Edit size={16} />
            </button>
            <button
            ...
            >
              {deletingPropertyId === property._id ? <Loader2 className="w-4 h-4 animate-spin text-red-500" /> : <Trash2 size={16} />}
            </button>
          </div>
        </div>
      </div>
